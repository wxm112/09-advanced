...
RSpec.describe Fruit, type: :model do
  ...
  describe 'A pear' do 
    ...
    it 'should remember what class it is using Single Table Inheritance (STI)' do 
      ...
      expect(pear).to_not eq(nil) # Existence/persistence
      expect(pear.class).to eq(Pear) # Class via STI
      expect(pear).to eq(@pear) # Same object
      expect(pear.is_a? Fruit).to eq(true) # Inheritance
